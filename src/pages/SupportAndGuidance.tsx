...
const SupportAndGuidance = () => (
  <div className="min-h-screen bg-[#FAF8F4] text-[#1A1A18]" style={{ fontFamily: "'DM Sans', system-ui, sans-serif", fontSize: 17, lineHeight: 1.75 }}>
    ...
    <nav className="sticky top-0 z-50 flex h-[62px] items-center justify-between border-b border-[#E4E2DA] bg-[#FAF8F4]/93 px-8 backdrop-blur-[14px]">
      <Link to="/" className="text-xl font-semibold tracking-tight text-[#1E6B5E]" style={{ fontFamily: "'Fraunces', Georgia, serif" }}>Collabor8</Link>
      <Link to="/signup" className="rounded-full bg-[#1E6B5E] px-[1.15rem] py-[0.45rem] text-[0.85rem] font-medium text-white transition-colors hover:bg-[#134840]">Open Collabor8</Link>
    </nav>

    <header className="mx-auto max-w-[680px] px-8 pb-16 pt-20 text-center">
      <span className="mb-5 inline-block text-[0.78rem] font-medium uppercase tracking-[0.12em] text-[#1E6B5E]">Resources for separated parents</span>
      <h1 className="mb-5 text-[clamp(2rem,5vw,3rem)] font-light leading-[1.15] tracking-tight text-[#1A1A18]" style={{ fontFamily: "'Fraunces', Georgia, serif" }}>
        Support &amp; financial guidance<br /><em className="italic text-[#1E6B5E]">after separation</em>
      </h1>
      ...
      <span className="inline-block max-w-[520px] rounded-[10px] bg-[#E8F5F2] px-5 py-[0.65rem] text-[0.9rem] italic leading-[1.6] text-[#6B6B64]">
    ...
    <section className="mx-auto max-w-[720px] px-8 pb-12 pt-4" aria-label="Support and guidance resources">
      ...
      <div className="mb-5 flex flex-wrap items-center justify-between gap-8 rounded-2xl bg-[#134840] px-10 py-9 text-white max-sm:flex-col max-sm:items-start max-sm:px-6 max-sm:py-6">
        <div>
        ...
        <Link to="/child-maintenance-calculator" className="inline-block flex-shrink-0 whitespace-nowrap rounded-full bg-white px-[1.6rem] py-[0.7rem] text-[0.9rem] font-medium text-[#134840] no-underline transition-all hover:-translate-y-0.5 hover:shadow-[0_6px_20px_rgba(0,0,0,0.2)]">
          Use the calculator &rarr;
        </Link>
      </div>

      {orgs.map((o) => (
        <div key={o.name} className="mb-5 rounded-2xl border border-[#E4E2DA] bg-white px-9 py-8 transition-shadow hover:shadow-[0_8px_32px_rgba(30,107,94,0.08)] max-sm:px-6 max-sm:py-6">
          <div className="mb-4 flex flex-wrap items-start justify-between gap-4">
            ...
            <span className="mt-[3px] flex-shrink-0 self-start whitespace-nowrap rounded-full bg-[#E8F5F2] px-[0.7rem] py-1 text-[0.73rem] font-medium uppercase tracking-[0.06em] text-[#134840]">{o.tag}</span>
          </div>
          ...
          <ul className="mb-6 flex list-none flex-col gap-[0.35rem] p-0">
            {o.helps.map((h) => (
              <li key={h} className="relative pl-[1.3rem] text-[0.9rem] leading-[1.5] text-[#6B6B64]">
                <span className="absolute left-0 top-[0.55em] h-[5px] w-[5px] rounded-full bg-[#1E6B5E]" />
                {h}
              </li>
            ))}
          </ul>
          <a href={o.href} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-[0.4rem] rounded-full border border-[#1E6B5E] px-[1.1rem] py-2 text-[0.88rem] font-medium text-[#1E6B5E] no-underline transition-all hover:bg-[#1E6B5E] hover:text-white">
            {o.cta} &#8599;
          </a>
        </div>
      ))}
    ...
    <div className="border-t border-[#E4E2DA] bg-[#FDFCFA] px-8 py-12">
      <div className="mx-auto flex max-w-[720px] flex-wrap items-center justify-between gap-8">
        ...
        <Link to="/signup" className="inline-block flex-shrink-0 whitespace-nowrap rounded-full bg-[#1E6B5E] px-7 py-3 text-[0.9rem] font-medium text-white no-underline transition-all hover:-translate-y-0.5 hover:bg-[#134840]">
          Try Collabor8 free
        </Link>
      </div>
    </div>
