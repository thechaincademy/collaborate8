...
const getNextPaymentDate = (
  ...
  if (frequency === "weekly" && dayOfWeek) {
    ...
    return nextDay(today, dayMap[dayOfWeek] ?? 1);
  }
...
const MaintenanceTab = () => {
  ...
  // Combine arrangement info with real payment history
  const transactions = [
    ...(activePayment
      ? [
          {
    ...
    ...paymentHistory.map((p) => ({
      name: p.type === "maintenance" ? "Maintenance payment" : p.type === "expense_reimbursement" ? "Expense payment" : "Payment",
      status: p.status === "completed" ? ("completed" as const) : ("pending" as const),
      amount: p.amount,
      date: format(new Date(p.created_at), "d MMM yyyy"),
      type: "outgoing" as const,
    })),
  ];
